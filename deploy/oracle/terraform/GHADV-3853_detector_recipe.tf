resource "oci_cloud_guard_detector_recipe" "ghadv-3853" {
  compartment_id  = var.compartment_id
  display_name    = "GHADV-3853: Spring AI's MilvusVectorStore#doDelete(List) implementation is vulnerable to filter-expression injection via unsanitized"
  ...
  detector_rules {
    ...
    condition_groups {
      group_name = "GHADV-3853_conditions"
      operator   = "OR"
