resource "oci_cloud_guard_detector_recipe" "oci-network-infrastructure-006" {
  compartment_id  = var.compartment_id
  display_name    = "OCI-NETWORK-INFRASTRUCTURE-006: CVE-2026-20079 \u2014 Secure Firewall Management Center (FMC) and Security Cloud Control (SCC) Firewall Management Exploitation"
  ...
  detector_rules {
    ...
    condition_groups {
      group_name = "OCI-NETWORK-INFRASTRUCTURE-006_conditions"
      operator   = "OR"
  ...
  }
  {
    ...
    labels = {
      rule_id       = "OCI-NETWORK-INFRASTRUCTURE-006"
